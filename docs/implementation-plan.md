...
| **L10** | **Sampling budget cut to fit 8 GB: `num_gop 8→5`, `num_mv`/`num_res` `59→16`** | The paper's N=8, M=59 was run on 32 GB V100s. On the 4070 it pins VRAM at **7710/8188 MiB (94%)** and the allocator thrashes — nvidia-smi shows 100% util at **29 W of 89 W**, i.e. stalling rather than computing, and epoch 1 projects to ~6 days.<br><br>Per batch the old budget pushed `2 × 8 × 59 = 944` residual images at 224²; as uint8 that is 142 MB, 568 MB once converted to float, doubled again by dropout, and retained by autograd. New budget is `2 × 5 × 16 = 160`, a **5.9× cut**.<br><br>`num_gop: 5` costs nothing — 35% of the 8 were measured as byte-identical duplicates. `num_mv/num_res: 16` is a real deviation: 16 of each GOP's 59 B/P frames, resampled at random every epoch, so the model still sees the whole GOP across training. Applied **identically to every variant**, so the comparison stays internally valid. Dependent settings: `n_bp` must equal `num_mv`, `max_v_len` must equal `2 × num_gop`. Worth an ablation row if the GPU budget ever allows. |
