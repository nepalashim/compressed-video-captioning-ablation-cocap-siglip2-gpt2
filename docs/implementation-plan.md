...
| **L3** | **`num_gop = 8`** (as specified) | **Measured**: `validate_data_pipeline.py` reports a **37.5% duplicated-GOP fraction** on the real clips — only ~5.0 of the 8 sampled GOPs are distinct, the rest byte-identical (GOP sampling happens after per-GOP B/P sampling, so duplicate indices yield identical tensors). Three of every eight I-frame/motion/residual encoder passes therefore produce features the model already has. `num_gop: 5` recovers that compute at no information cost, and is **confirmed**: at 5 the validator reports a **4.0% duplicated fraction** against 34.8% at 8 (measured over 1200 samples, about 120 distinct clips). The residual 4% is clips that hold fewer than five GOPs at all, which no choice of N avoids. An earlier 0.0% reading came from 120 samples, which is only ~12 clips once unfold_sentences expands each caption into its own sample. Selected by `budget=laptop_8gb` (see L10). |
...
| **L11** | **Pre-extraction cache** — ✅ **BUILT** | `tools/pre_extract.py` parses each clip once and caches the four artifacts the reader's `pre_extract` path expects. Measured on the full subset: **3.4 MB/clip**, 16.85 GB for 5,000 train clips and 3.38 GB for 1,000 val, at 12.5 clips/s — **~8 minutes total, zero failures**. Select with `reader=pre_extract`; `reader=decode` keeps the original per-sample parse. Far cheaper than the 36–84 GB first estimated, because JPEG compresses residuals (mostly flat grey) and lz4 compresses the two dead motion channels. |
